...
public class MainAnalyser {
	...
	private void saveMap() {
		...
		for (HookFrame hook : hookMap.values()) {
			hookArray.add(hook.toJson());
		}
		try (Writer writer = new BufferedWriter(new FileWriter(FileDownloader.getContentDirectory() + "hooks.txt"))) {
			hookArray.writeJSONString(writer);
		} catch (IOException e) {
			e.printStackTrace();
		}
		System.out.println("Saved to hook file.");
	}
